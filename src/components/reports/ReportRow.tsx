...
export const ReportRow = memo(function ReportRow({
  ...
  const excerpt = useMemo(() => {
    ...
    if (lastFinding === null) return t('reports_no_findings_yet'); // empty
  ...
  const menuItems = useMemo(() => {
    ...
    if (onToggleEnabled) {
      items.push({
        label: report.enabled ? t('reports_row_pause') : t('reports_row_enable'),
    ...
    return items;
  }, [onEdit, onToggleEnabled, onDelete, report.id, report.enabled, t]);
    ...
    <>
      <div
        className={`
          ...
          ${interactive ? 'cursor-pointer hover:bg-[var(--color-bg-hover)] active:bg-[var(--color-accent)]/10' : ''}
