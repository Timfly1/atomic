...
export class HttpTransport implements Transport {
  ...
  async uploadEmbeddedImage(atomId: string, file: File): Promise<{ id: string; content_type: string }> {
    if (this.authExpired) {
      throw new Error('Authentication expired. Please reconnect with a valid token.');
    }
    if (!this.config.baseUrl) {
  ...
  async deleteEmbeddedImage(atomId: string, imageId: string): Promise<void> {
    if (this.authExpired) {
      throw new Error('Authentication expired. Please reconnect with a valid token.');
    }
    if (!this.config.baseUrl) {
    ...
    const url = `${this.config.baseUrl}/api/atoms/${encodeURIComponent(atomId)}/embedded-images/${encodeURIComponent(imageId)}`;
    const headers: Record<string, string> = {
      'Authorization': `Bearer ${this.config.authToken}`,
    };
